...
#define SK_B32_SHIFT 1
#define SK_R32_SHIFT 1
...
bool JPEGCodec::Decode(const unsigned char* input, size_t input_size,
  ...
  jpeg_save_markers(&cinfo, exifMarker, 0xFFFF);
  ...
  *h = cinfo.output_height;
  ...
  qcms_profile* deviceProfile = qcms_profile_sRGB();

  JOCTET* profile;
  unsigned profileLength;
  qcms_profile* inputProfile = NULL;

  if (read_icc_profile(&cinfo, &profile, &profileLength))
  {
      inputProfile = qcms_profile_from_memory(profile, profileLength);
  }
  else
  {
      inputProfile = qcms_profile_from_path("D:/AdobeRGB1998.icc");
  }
 
  qcms_transform* pTransform = NULL;
  if (inputProfile != NULL && deviceProfile != NULL && !qcms_profile_match(inputProfile, deviceProfile))
  {
      pTransform = qcms_transform_create(inputProfile, dataFormat, deviceProfile, dataFormat, QCMS_INTENT_PERCEPTUAL);
  }
  ...
  J_COLOR_SPACE real = cinfo.out_color_space;
  ...
  for (int row = 0; row < static_cast<int>(cinfo.output_height); row++) {
    ...
    if (pTransform)
    {
        qcms_transform_data_type(pTransform, rowptr, rowptr, cinfo.output_width, real == JCS_EXT_BGRA ? QCMS_OUTPUT_BGRX : QCMS_OUTPUT_RGBX);
    }
  ...
  if (pTransform)
  {
      qcms_transform_release(pTransform);
  }
